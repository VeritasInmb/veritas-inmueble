...
export const AgencyCard: React.FC<AgencyCardProps> = ({ agency, onSelect }) => {
    ...
    return (
        <div className={`rounded-[2rem] shadow-xl shadow-slate-200/50 transition-all duration-300 group cursor-pointer relative energy-card-wrapper`} onClick={() => onSelect(localAgency)}>
            <div className="bg-white rounded-[1.9rem] p-4 h-full w-full relative z-10">
                <div className="absolute top-4 right-4 flex items-center gap-1.5 bg-slate-100/80 backdrop-blur-md px-3 py-1.5 rounded-full border border-slate-200/50 shadow-sm">
                ...
                <div className="mt-4 flex flex-col items-center text-center">
                    <div className="w-14 h-14 rounded-2xl bg-slate-50 flex items-center justify-center overflow-hidden shadow-inner mb-3 border border-slate-100">
                        {localAgency.imageUrl ? <img src={localAgency.imageUrl} alt={localAgency.nombre} className="w-full h-full object-cover" /> : <BuildingOfficeIcon className="w-6 h-6 text-slate-300" />}
                    </div>
                    <h3 className="text-base font-black text-slate-900 leading-tight tracking-tight mb-1">{localAgency.nombre}</h3>
                    <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest">{localAgency.estado}</p>
                    
                    <div className="flex items-center justify-center gap-2 my-3 w-full">
                        <div className="flex flex-col items-center px-2 py-1.5 bg-slate-50 rounded-2xl flex-1 border border-slate-100/50">
                            <span className={`text-2xl font-black tracking-tighter ${scoreInfo.textColor}`}>{calculatedScore}</span>
                            <span className="text-[7px] uppercase font-black text-slate-400 tracking-widest mt-0.5">Score</span>
                        </div>
                        {!ratingData.loading && (
                            <div className="flex flex-col items-center px-2 py-1.5 bg-slate-50 rounded-2xl flex-1 border border-slate-100/50">
                                <div className="flex items-center gap-1 mb-0.5">
                                    <span className="text-lg font-black tracking-tighter text-slate-900">{ratingData.socialVerdict.toFixed(1)}</span>
                                    <StarIcon className="w-3 h-3 text-yellow-400" filled={true} />
                                </div>
                                <span className="text-[7px] uppercase font-black text-slate-400 tracking-widest mt-0.5 text-center">{ratingData.count} Opiniones</span>
                            </div>
                        )}
                    </div>
